...
{
    private readonly int[] BoxPermutationMatrix = new int[32]
    {
    ...
    private readonly int[] initialPermutationVector =
    {
    ...
    private readonly int[] finalPermutationVector = new int[64]
    {
    ...
    private readonly int[] expansionVector =
    {
    ...
    public BitArray Round(BitArray input, BitArray key)
    {
        ...
        var boxed = BoxSubstitution(xored);
        var permutated = FinalBoxPermutation(boxed);
        return permutated;
    }
    private BitArray Expand(BitArray input)
    {
        ...
        for (var i = 0; i < 48; i++)
            array[i] = input[expansionVector[i] - 1];

        return new BitArray(array);
    }
    private BitArray BoxSubstitution(BitArray input)
    {
        string r;
        int row, col;
        string c;
        ...
        for (var i = 0; i < boxes.Length; i++)
        {
            ...
            c = box[1].ToBinaryDigit() + box[2].ToBinaryDigit() + box[3].ToBinaryDigit() + box[4].ToBinaryDigit();
    ...
    public BitArray InitialPermutation(BitArray bitArray)
    {
        ...
        for (var i = 0; i < 64; i++)
            array[i] = bitArray[initialPermutationVector[i] - 1];
        return new BitArray(array);
    }
    ...
    public BitArray FinalPermutation(BitArray bitArray)
    {
        var array = new bool[bitArray.Length];
        for (var i = 0; i < 64; i++)
            array[i] = bitArray[finalPermutationVector[i] - 1];
        return new BitArray(array);
    }

    private BitArray FinalBoxPermutation(BitArray bitArray)
    {
        var array = new bool[bitArray.Length];
        for (var i = 0; i < 32; i++)
            array[i] = bitArray[BoxPermutationMatrix[i]];
        return new BitArray(array);
    }
}